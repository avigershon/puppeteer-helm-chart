...
gcp_create_cluster () {
  ...
  gcloud beta container --project "crawler-249816" clusters create "${CLUSTER_NAME}" --zone "asia-east2-a" --username "admin" --cluster-version "1.12.8-gke.10" --machine-type "n1-standard-2" --image-type "COS" --disk-type "pd-standard" --disk-size "10" --scopes "https://www.googleapis.com/auth/cloud-platform" --preemptible --num-nodes "1" --enable-cloud-logging --enable-cloud-monitoring --no-enable-ip-alias --network "projects/crawler-249816/global/networks/default" --subnetwork "projects/crawler-249816/regions/asia-east2/subnetworks/default" --enable-autoscaling --min-nodes "1" --max-nodes "4" --addons HorizontalPodAutoscaling,HttpLoadBalancing --enable-autoupgrade --enable-autorepair
